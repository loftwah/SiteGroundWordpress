 ...
 * @since 1.0.0
 */
class Autoremove_Attachments_Update_PHP {
		...
		if ( current_user_can( 'manage_options' ) ) {
			?>
				<div class="error polygon-warning">
					...
					<p>
						<?php printf( esc_html__( 'Autoremove Attachments can not run on PHP versions older than %1$s. You are running on version %2$s which has serious security and performance issues.', 'autoremove-attachments' ), $this->minimum_version, PHP_VERSION ); ?>
						<br>
						<?php printf( esc_html__( 'Please ask your hosting provider to help you upgrade. We recommend PHP %1$s or newer.', 'autoremove-attachments' ), $this->recommended_version ); ?>
					</p>
					<p></p>
				</div>
			<?php
